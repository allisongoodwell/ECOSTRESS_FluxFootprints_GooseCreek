...
save_name= 'ECOSTRESS_footprintETvalues_'+NxNstr+'.csv'
...
for date_val in Raster_List_JPLInst:
    ...
    ax4.legend(['10m LE','25m LE','overpass','ET_NxN','ET_1x1','ET_ffp25','ET_ffp10'],loc = 'upper left')
    ...
    for i,(ax,raster) in enumerate([(ax2, rasterJPLdaily),(ax3, rasterAlexi)]):
        #c0 = ax.imshow(raster,extent = raster_extent,cmap = 'viridis_r',vmin=minval[i],vmax=maxval[i])
        c0 = ax.imshow(raster,extent = raster_extent,cmap = 'viridis_r',vmin=1,vmax=5)
...
data=np.asarray([date_list,good_image,ET10_mm,ET25_mm,LE10_list, LE25_list,
                 ...
                 J_inst_corn, J_inst_soy, J_daily_corn, J_daily_soy, A_daily_corn, A_daily_soy, ETcorn, ETsoy]).T
...
df_ALL = pd.DataFrame(data=data,
                  columns = ['Date','good','ET10day','ET25day','LE10inst','LE25inst',
                             ...
                             'JinstCorn','JinstSoy','JdayCorn','JdaySoy','AlexiCorn','AlexiSoy','ETTowerCorn','ETTowerSoy'])

for i,colname in enumerate(df_ALL):
    if colname != 'Date':
        df_ALL[colname] = pd.to_numeric(df_ALL[colname],errors='coerce')
 
df_ALL['TowerCropDiff']=df_ALL['ETTowerCorn']-df_ALL['ETTowerSoy']
...
df_little = df_ALL[['TowerCropDiff','AlexiCropDiff','JdayCropDiff']]

print(df_little.describe())

print(df_little.corr())

pvals = calculate_pvalues(df_little)
